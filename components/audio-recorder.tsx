...
// @ts-ignore
import WhisperWorker from "../lib/worker.ts?worker";
...
export function AudioRecorder({ onTranscriptionComplete }: AudioRecorderProps) {
    ...
    if (workerRef.current) {
      workerRef.current.onmessage = (e: MessageEvent) => {
        ...
        if (status === "start") {
          ...
          setCurrentTranscript(output);
        } else if (status === "complete") {
          const newSegment: TranscriptionSegment = {
            id: Date.now().toString(),
            text: Array.isArray(output) ? output.join(" ") : output,
  ...
  const stopRecording = () => {
    ...
    // Send audio to Whisper worker
    const audioBlob = new Blob(audioChunksRef.current, { type: "audio/webm" });
    workerRef.current?.postMessage({
      type: "generate",
      data: { audio: audioBlob, language: "en" },
    });
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Record your lecture and PrepPal will transcribe it automatically
          </CardDescription>
        </CardHeader>
              ...
              <div className="space-y-3">
                ...
                {currentTranscript && (
                  <div className="flex gap-3">
                    <Badge
                      variant="outline"
                      className="text-xs shrink-0 mt-1 bg-accent/10"
                    >
                      {formatTime(recordingTime)}
                    </Badge>
